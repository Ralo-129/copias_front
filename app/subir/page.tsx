    ...
    useEffect(() => {
     ...
     return (
        <div className="min-h-screen bg-background flex justify-center px-4 py-10">
            <UploadForm grado={grado} seccion={seccion} />
        </div>
    );
}

function UploadForm({ grado, seccion }: { grado: string; seccion: string }) {
    ...
    async function handleSubmit(e: React.FormEvent) {
        ...
        if (data.ok) {
            ...
            setError(data.error ?? 'Ocurrií un error al enviar la solicitud ');
        }
    }
